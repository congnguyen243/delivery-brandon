...
const actionConfirm = functions.https.onRequest(async (req, res) => {
    if(req.method.toUpperCase() == 'GET') {
        ...
        if ('string' !== typeof the_request_id
            ...
            res_cl.get().then((doc) => {
                if (doc.exists) {
                    ...
                    if(doc.data().status) {
                        ...
                        functions.logger.info(`admin updated status request #${the_request_id}`, {data: the_request});
                        res_cl.update({ status: the_request });
                        let mailOptions = template.template_confirmation(user, the_email, the_request, the_request_id);
                        ...
                        transporter.sendMail(mailOptions, (error, data) => {
                            ...
                            functions.logger.info(`Sent mail for request #${the_request_id} successful`, {data: data});
                        });
                    ...
                    res.status(400).send(`The requesr #${the_request_id} not found`);
                }
            }).catch((error) => {
